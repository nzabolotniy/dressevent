...
function commentlist($comment, $args, $depth) {
    ...
    <li id="li-comment-<?php comment_ID() ?>">
        <div id="comment-<?php comment_ID(); ?>" <?php comment_class('comment_item clear'); ?>>
            <div class="comment_meta">Опубликовано <?php printf( __('%1$s'), get_comment_date('d.m.Y')); ?> <?php printf(__('<cite class="fn">%s</cite>'), get_comment_author_link()) ?></div>
...
register_taxonomy(
    ...
    'portfolio',
    array(
        'label' => __('Рубрики портфолио'),
        'singular_label' => __('Рубрика портфолио'),
        'hierarchical' => true,
        'query_var' => true,
        'rewrite' => true,
...
function commentdata_fix($commentdata) {
    ...
    if ($commentdata['comment_content'] == 'Ваш отзыв') {
