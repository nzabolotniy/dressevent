<?php if ( have_posts() ) : ?>
    ...
    <div <?php post_class('post_list clear'); ?> id="post-<?php the_ID(); ?>">
        <h2><a href="<?php the_permalink() ?>" title="<?php printf( esc_attr__('Постоянная ссылка: %s'), the_title_attribute('echo=0') ); ?>" rel="bookmark"><?php the_title(); ?></a></h2>
        <div class="post_list_meta">
            <p class="post_date">Опубликовано: <?php the_time(__('d M Y')) ?> Автор: <?php the_author() ?></p>
            <p class="post_cat"><?php the_category(', '); ?></p>
            <p class="post_comms"><?php comments_popup_link(__('Ваш отзыв'), __('1 отзыв'), __('Отзывов (%)'), '', __('Отзывы закрыты') ); ?></p>
            <div class="post_share">&mdash;<br /><a href="javascript: void(0);" class="sharethis">Поделитесь с другими</a>
                <ul class="sharelist">
        ...
        <div class="post_content">
            ...
            <p class="more">&mdash; <a href="<?php the_permalink() ?>#more">Читать полностью</a></p>
        </div>
    </div>
